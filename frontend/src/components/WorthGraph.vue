...
const renderGraph = () => {
  ...
  // Axes
  const xAxis = d3.axisBottom(xScale).tickFormat(d3.timeFormat("%b %d")).ticks(6);
  ...
  svg
    ...
    .on("mousemove", (event, layer) => {
      ...
      tooltip
        ...
        .style("left", `${event.pageX - 300}px`) // Adjusted position
        .style("top", `${event.pageY - 300}px`) // Adjusted position
    })
    .on("mouseout", () => {
